...
public class NewVendingSerialPort {
    ...
    public boolean Protocal(byte ch) {
        ...
        if ((state == DATASTATE.IDEL) && (ch == H1))//0xEF
            state = DATASTATE.FIND_H1;
        else if ((state == DATASTATE.FIND_H1) && (ch == H2))//0xEE
            state = DATASTATE.FIND_H2;
        else if ((state == DATASTATE.FIND_H2) && (ch == H3))//0xFE
        {
        ...
        public void run() {
            ...
            while (!isStop && !isInterrupted()) {
                try {
                    ...
                    if (Protocal(hexStringToByte(bytesToHexString(buffer)))) {
                        CMD(ReSign);
                    }
